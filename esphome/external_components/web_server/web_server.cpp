...
bool WebServer::canHandle(AsyncWebServerRequest *request) const {
  ...
  if (url == ESPHOME_F("/"))
    return true;
#ifdef USE_WEBSERVER_APP_HTML_INCLUDE
  if (url == ESPHOME_F("/esphome") || url == ESPHOME_F("/esphome/") || url == ESPHOME_F("/esphome/index.html"))
    return true;
  // Match any embedded static asset by URL
  {
    std::string url_str = url.c_str();
    for (size_t i = 0; i < STATIC_FILES_COUNT; i++) {
      if (url_str == STATIC_FILES[i].url)
        return true;
    }
  }
#endif
...
#ifdef USE_WEBSERVER_APP_HTML_INCLUDE
  return true;  // Catch all remaining URLs for SPA routing
#else
  return false;
#endif
}
...
#ifdef USE_WEBSERVER_APP_HTML_INCLUDE
    // Route to static asset or SPA fallback
    std::string url_str = url.c_str();
...
void WebServer::handle_app_request(AsyncWebServerRequest *request) {
  AsyncWebServerResponse *response = request->beginResponse(
      200, "text/html", ESPHOME_WEBSERVER_APP_HTML_INCLUDE, ESPHOME_WEBSERVER_APP_HTML_INCLUDE_SIZE);
...
#ifdef USE_ESP32
  char url_buf[AsyncWebServerRequest::URL_BUF_SIZE];
  std::string url = request->url_to(url_buf).c_str();
#else
