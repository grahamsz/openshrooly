...
void AsyncEventSourceResponse::process_buffer_() {
  ...
  if (bytes_sent == HTTPD_SOCK_ERR_FAIL) {
    // Asynchronous responses are no longer owned by the HTTP server's request
    // loop, so a send error does not make it close the session automatically.
    this->request_close_();
    return;
  }
  if (bytes_sent <= 0) {
...
void AsyncEventSourceResponse::request_close_() {
  ...
  // Queue the close on the HTTP server task. Its session cleanup callback will
  // set fd_ to zero, after which AsyncEventSource::loop() can safely delete this
  // response without leaving the server with a dangling session context.
  const esp_err_t err = httpd_sess_trigger_close(this->hd_, fd);
  if (err != ESP_OK) {
    ESP_LOGW(TAG, "Failed to request EventSource session close for fd %d: %s", fd, esp_err_to_name(err));
    return;
  }
...
bool AsyncEventSourceResponse::try_send_nodefer(const char *message, size_t message_len, const char *event, uint32_t id,
                                                uint32_t reconnect) {
  if (this->close_requested_ || this->fd_.load() == 0) {
    return false;
  }
